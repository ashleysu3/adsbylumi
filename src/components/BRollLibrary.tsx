...
export function BRollLibrary({ brandId, clips, onUpdate }: BRollLibraryProps) {
  ...
  const handleUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of validFiles) {
      ...
      const storagePath = `${brandId}/${id}-${file.name}`;
    ...
    if (newClips.length > 0) {
      ...
      const { error } = await supabase
        .from("brands")
        .update({ broll_library: updated as any })
        .eq("id", brandId);
      ...
      if (error) {
        ...
        onUpdate(updated);
        toast.success(`${newClips.length} clip${newClips.length > 1 ? "s" : ""} uploaded`);
      }
    }
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Upload your everyday b-roll clips (under 30 seconds each). Lumi will auto-pair them with text overlays in the production checklist.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-lg p-6 cursor-pointer hover:border-primary/50 hover:bg-muted/30 transition-colors">
          ...
          <span className="text-sm text-muted-foreground">
            {uploading ? "Uploading..." : "Click to upload video clips (max 250MB each)"}
          </span>
          <input
            type="file"
            accept="video/*"
        ...
        {clips.length === 0 && (
          <div className="text-center py-4">
            <p className="text-sm text-muted-foreground">
              💡 Upload clips like: pouring coffee, typing on a laptop, walking the dog, fixing your hair, driving...
            </p>
          </div>
        )}
      </CardContent>
