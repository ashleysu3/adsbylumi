...
export function CampaignFlowBreadcrumb({
  ...
  return <nav aria-label="Campaign progress" className={cn("border-b border-border bg-background/95 backdrop-blur-sm", className)}>
      <div className="container mx-auto px-4 py-[16px] pt-0 pb-[18px]">
        <ol className="flex items-center justify-center gap-2 flex-wrap">
          ...
          return <li key={step.id} className="flex items-center gap-2">
                <Button variant="ghost" size="sm" onClick={() => handleStepClick(step)} disabled={isDisabled} className={cn("flex items-center gap-2 h-auto py-2 px-3 transition-colors", isCurrent && "bg-primary/10 text-primary font-semibold", isCompleted && "text-muted-foreground hover:text-foreground", isDisabled && "opacity-40 cursor-not-allowed")}>
                  {isCompleted ? <CheckCircle2 className="h-4 w-4 text-green-600 dark:text-green-400" /> : <Circle className={cn("h-4 w-4", isCurrent && "fill-primary text-primary", !isCurrent && !isCompleted && "text-muted-foreground")} />}
