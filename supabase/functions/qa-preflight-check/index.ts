...
async function checkSpellingGrammar(creativeJson: any, productionItems: any[], selectedCopy?: any): Promise<CheckResult> {
  try {
    ...
    // 4. Advanced Build shared variations (selectedCopy.shared_variations)
    const sharedVariations: any[] = selectedCopy?.shared_variations || [];
    sharedVariations.forEach((v: any, i: number) => {
      const label = `Variation ${i + 1}`;
      if (v?.headline) copyToCheck.push({ field: 'headline', text: v.headline, location: label });
      if (v?.primary_text) copyToCheck.push({ field: 'primary_text', text: v.primary_text, location: label });
