...
serve(async (req) => {
  ...
  try {
    ...
    const r = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "google/gemini-3.1-flash-image-preview", messages: [{ role: "user", content: full }], modalities: ["image"] }),
    });
    const d = await r.json();
    const item = d?.data?.[0];
    const generatedDataUrl = d?.choices?.[0]?.message?.images?.[0]?.image_url?.url;
    let bytes: Uint8Array | null = null;
    if (generatedDataUrl?.startsWith("data:")) {
      const b64 = generatedDataUrl.split(",")[1];
      bytes = Uint8Array.from(atob(b64), (c) => c.charCodeAt(0));
    } else if (item?.b64_json) {
      bytes = Uint8Array.from(atob(item.b64_json), (c) => c.charCodeAt(0));
    } else if (generatedDataUrl || item?.url) {
      const imgRes = await fetch(generatedDataUrl || item.url);
