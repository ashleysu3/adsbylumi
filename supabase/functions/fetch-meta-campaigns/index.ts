...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (dateRangeStart && dateRangeEnd) {
      ...
      const insightsUrl = `https://graph.facebook.com/v18.0/${metaAccountId}/insights?level=campaign&time_range={"since":"${dateRangeStart}","until":"${dateRangeEnd}"}&fields=campaign_id,campaign_name,spend&filtering=[{"field":"spend","operator":"GREATER_THAN","value":"0"}]&limit=500&access_token=${metaAccessToken}`;
