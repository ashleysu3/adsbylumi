...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const adsUrl = `https://graph.facebook.com/v18.0/${campaignId}/ads?fields=id,name,status,creative{object_story_spec,asset_feed_spec}&limit=50&access_token=${metaAccessToken}`;
    ...
    if (urls.size === 0) {
      for (const ad of (adsData.data || []).slice(0, 5)) {
        try {
          const creativeUrl = `https://graph.facebook.com/v18.0/${ad.id}?fields=creative{effective_object_story_id,object_story_spec}&access_token=${metaAccessToken}`;
