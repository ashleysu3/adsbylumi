  ...
  try {
    ...
    if (hasEnoughData && ctr !== null && ctr < alertThresholds.ctr_warning) {
      recommendations.push({
        ...
        title: 'Ad creative isn\'t resonating',
        description: `CTR of ${Number(ctr).toFixed(2)}% is below the ${alertThresholds.ctr_warning}% benchmark. Your ads aren't getting clicks — test new hooks, angles, or formats.`,
        impact: 'Fresh ad creative can dramatically improve engagement',
    ...
    if (hasEnoughData && frequency !== null && frequency >= fatigueThreshold) {
      recommendations.push({
        ...
        description: `Frequency of ${Number(frequency).toFixed(1)} means people are seeing your ads too many times. Swap in fresh ad creative to re-engage your audience.`,
        impact: 'Prevent declining ad performance from creative fatigue',
        confidence: 'high',
        requiresDoubleApproval: false,
        actionPayload: { workspaceId, brandId },
        userAction: true,
    ...
    if (hasEnoughData && campaignMetrics.cpp && campaignMetrics.cpp > 100) {
      recommendations.push({
        ...
        description: `CPP of $${Number(campaignMetrics.cpp).toFixed(2)} is elevated. Test new ad angles, stronger hooks, or different creative formats to bring this down.`,
        impact: 'Lower acquisition cost with stronger ad creative',
        confidence: 'medium',
        requiresDoubleApproval: false,
        actionPayload: { workspaceId, brandId },
        userAction: true,
