      ...
      if (hasGoals && primaryMet === false && primaryClose === false) {
        if (ctr < 0.01) {
          ...
          recommendations.push({
            ...
            description: `Your goal is ${formatThreshold(primaryKpi, primaryThreshold!)} but you're at ${formatKpiValue(primaryKpi, primaryValue)}. CTR is ${ctr.toFixed(2)}% — people are clicking but not taking action. The issue is post-click: landing page copy, load speed, or offer clarity. Pause underperforming ads and review your landing page.`,
            impact: 'Fixing post-click conversion can bring CPL to goal without new creative',
            confidence: 'high',
            requiresDoubleApproval: false,
            actionPayload: {},
            priority: priority++,
