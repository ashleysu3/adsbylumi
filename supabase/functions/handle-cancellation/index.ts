...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const brand of (brands || [])) {
      ...
      try {
        const campaignsUrl = `https://graph.facebook.com/v25.0/${metaAccountId}/campaigns?fields=id,name,status&filtering=[{"field":"status","operator":"IN","value":["ACTIVE"]}]&limit=500&access_token=${metaAccessToken}`;
