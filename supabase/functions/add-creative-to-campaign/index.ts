  ...
  try {
    ...
    const isAdSetValidForAccount = async (candidateAdSetId: string): Promise<boolean> => {
      try {
        const adSetCheckRes = await fetch(
          `https://graph.facebook.com/v25.0/${candidateAdSetId}?fields=id,account_id,status,effective_status,campaign_id&access_token=${metaAccessToken}`
        );
    ...
    if (!adSetId && campaignId) {
        ...
        for (const adId of existingAdIds) {
          try {
            const adInfoRes = await fetch(
              `https://graph.facebook.com/v25.0/${adId}?fields=adset_id&access_token=${metaAccessToken}`
            );
      ...
      if (!adSetId) {
        const adSetsRes = await fetch(
          `https://graph.facebook.com/v25.0/${campaignId}/adsets?fields=id,name,status,effective_status&limit=10&access_token=${metaAccessToken}`
        );
    ...
    try {
      ...
      const adsRes = await fetch(
        `https://graph.facebook.com/v25.0/${adSetId}/ads?fields=creative{object_story_spec,url_tags,tracking_specs,asset_feed_spec}&limit=1&access_token=${metaAccessToken}`
      );
    ...
    try {
      const adSetMetaRes = await fetch(
        `https://graph.facebook.com/v25.0/${adSetId}?fields=id,is_dynamic_creative&access_token=${metaAccessToken}`
      );
    ...
    const getAdSetDynamicMode = async (candidateAdSetId: string): Promise<boolean | null> => {
      try {
        const modeRes = await fetch(
          `https://graph.facebook.com/v25.0/${candidateAdSetId}?fields=id,is_dynamic_creative&access_token=${metaAccessToken}`
        );
    ...
    const cloneAdSetForAsset = async (assetName: string): Promise<string | null> => {
      try {
        const cloneRes = await fetch(
          `https://graph.facebook.com/v25.0/${adSetId}/copies`,
          {
            method: 'POST',
