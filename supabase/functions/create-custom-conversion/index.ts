...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const pixelResponse = await fetch(
      `https://graph.facebook.com/v25.0/act_${accountId}/adspixels?fields=id,name&access_token=${metaAccessToken}`
    );
    ...
    const ccResponse = await fetch(
      `https://graph.facebook.com/v25.0/act_${accountId}/customconversions`,
      {
        method: 'POST',
