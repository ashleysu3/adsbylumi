-- stock_broll_clips: global curated library of stock video clips used to auto-assemble
-- a b-roll ad for brand-new visitors who haven't uploaded any footage of their own yet.
...
CREATE POLICY "Signed-in users can view stock broll clips"
  ON public.stock_broll_clips FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Admins can insert stock broll clips"
  ON public.stock_broll_clips FOR INSERT
  TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can update stock broll clips"
  ON public.stock_broll_clips FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can delete stock broll clips"
  ON public.stock_broll_clips FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role));

CREATE INDEX idx_stock_broll_clips_categories ON public.stock_broll_clips USING GIN (categories);

-- Storage bucket for the actual clip + poster-frame files
INSERT INTO storage.buckets (id, name, public)
VALUES ('stock-broll', 'stock-broll', true)
